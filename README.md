...
Askaiser.Marionette is a **test automation framework based on image and text recognition**. It includes a C# source generator that allows you to quickly interact with properties generated by images from your project or elsewhere.
...
GetCurrentMonitor()
GetMonitors()
SetMonitor(int monitorIndex)
SetMonitor(MonitorDescription monitor)
...
KeyPress(params VirtualKeyCode[] keyCodes)
KeyDown(params VirtualKeyCode[] keyCodes)
KeyUp(params VirtualKeyCode[] keyCodes)
...
### Key press with sleep after

```csharp
KeyPress(VirtualKeyCode[] keyCodes, TimeSpan sleepAfter)
KeyDown(VirtualKeyCode[] keyCodes, TimeSpan sleepAfter)
KeyUp(VirtualKeyCode[] keyCodes, TimeSpan sleepAfter)
```
